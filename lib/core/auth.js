...
export default class Auth {
  ...
  async setStrategy (name) {
    if (name === await this.$storage.getUniversal('strategy')) {
      return Promise.resolve()
    }

    // Set strategy
    await this.$storage.setUniversal('strategy', name)
  ...
  async setUserToken (token) {
    if (!this.strategy.setUserToken) {
      await this.setToken(this.strategy.name, token)
      return Promise.resolve()
    }
  ...
  async reset () {
    if (!this.strategy.reset) {
      ...
      await this.setToken(this.$state.strategy, false)
      await this.setRefreshToken(this.$state.strategy, false)
      return Promise.resolve()
    }
  ...
  async getToken (strategy) {
    ...
    return await this.$storage.getUniversal(_key)
  }

  async setToken (strategy, token) {
    const _key = this.options.token.prefix + strategy

    return await this.$storage.setUniversal(_key, token)
  }

  async syncToken (strategy) {
    const _key = this.options.token.prefix + strategy

    return await this.$storage.syncUniversal(_key)
  }
  ...
  async getRefreshToken (strategy) {
    const _key = this.options.refresh_token.prefix + strategy

    return await his.$storage.getUniversal(_key)
  }

  async setRefreshToken (strategy, refreshToken) {
    const _key = this.options.refresh_token.prefix + strategy

    return await this.$storage.setUniversal(_key, refreshToken)
  }

  async syncRefreshToken (strategy) {
    const _key = this.options.refresh_token.prefix + strategy
  ...
  async requestWith (strategy, endpoint, defaults, withResponse) {
    const token = await this.getToken(strategy)
  ...
  async redirect (name, noRouter = false) {
    ...
    if (this.options.rewriteRedirects) {
      if (name === 'login' && isRelativeURL(from) && !isSameURL(to, from)) {
        await this.$storage.setUniversal('redirect', from)
      }

      if (name === 'home') {
        const redirect = this.$storage.getUniversal('redirect')
        await this.$storage.setUniversal('redirect', null)
